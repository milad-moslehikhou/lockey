...
from rest_framework.serializers import DjangoValidationError, ValidationError
...
from apps.user.models import PasswordRecord, User
...
class UserViewSet(ModelViewSet):
    ...
    @action(methods=["GET"], url_name="all-permissions", url_path="all-permissions", detail=True)
    def get_all_permissions(self, request, pk=None):
        user = get_object_or_404(User, pk=pk)
        permissions = Permission.objects.filter(Q(user=user) | Q(group__in=user.groups.all()))
        serializer = PermissionSerializer(permissions, many=True)
        return Response(serializer.data)

    @action(methods=["GET"], url_name="all-grants", url_path="all-grants", detail=True)
    def get_all_grants(self, request, pk=None):
        user = get_object_or_404(User, pk=pk)
        grants = CredentialGrant.objects.filter(Q(user=user) | Q(group__in=user.groups.all()))
        serializer = CredentialGrantSerializer(grants, many=True)
        return Response(serializer.data)

    @action(methods=["POST"], url_name="set-password", url_path="set-password", detail=True)
    def set_userpassword(self, request, pk=None):
        ...
        user.save()
        pass_record = PasswordRecord(user=user, password=user.password)
        pass_record.save()
        return Response(status=status.HTTP_200_OK)

    @action(methods=["POST"], url_name="change-password", url_path="change-password", detail=True)
